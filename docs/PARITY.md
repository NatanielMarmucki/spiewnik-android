...
| Wyszukiwanie: polskie znaki | Brak normalizacji diakrytyków | Brak normalizacji diakrytyków | **—** |
...
| **Blokada wygaszania ekranu** | Brak | Zawsze włączona na ekranie szczegółów (`wakelock_plus`). **Znany błąd, potwierdzony testem:** po `pushReplacement` (przejście do numeru, przeciągnięcie na następną lub poprzednią pieśń) blokada się wyłącza, choć ekran szczegółów jest otwarty, bo `dispose` starego ekranu wywołuje `disable` po `enable` nowego. Do naprawy w grupie A (A3); odtworzenie w `test/song_detail_view_wakelock_test.dart` (testy z `skip`) | **B** |
